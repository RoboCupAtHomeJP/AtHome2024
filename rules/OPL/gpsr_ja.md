...
スマートスピーカーのように，ホストが要求する任意のコマンドを実行するようにロボットに依頼される．
...
1. コマンドフェーズ (Command Phase)
    ...
    1. **ホストへの移動 (Navigation to Operator)**: ロボットは*Start Location*から*Instruction point*まで移動する．
